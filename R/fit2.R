#' @include fit_edgeR.R fit_fbseq2.R fit_Niemi.R
NULL

#' @title Function \code{fit2}
#' @description run benchmark methods on simulation objects for priors_mcmc. uses fit_fbseq2
#' @export
...
fit2 = function(path, benchmarks = c("edgeR", "Niemi"), fbseq_methods = c("fullybayes", "ebayesFromFullybayes", "ebayesFromStarts", "ebayesFromTruth"), 
  ...
  for(prior in priors) for(fbseq_method in fbseq_methods) for(f in files){
    ...
    if(is.null(o$analyses[[method]])){
      o$analyses[[method]] = fit_fbseq2(o, fbseq_method, prior, debug, configs = configs, zeronormfactors = zeronormfactors)
      saveRDS(o, p)
    }
  }
